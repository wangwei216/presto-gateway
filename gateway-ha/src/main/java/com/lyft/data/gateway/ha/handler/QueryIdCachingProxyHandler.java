...
import static com.lyft.data.gateway.ha.router.RoutingGroupSelector.ALTERNATE_ROUTING_GROUP_HEADER;
...
public class QueryIdCachingProxyHandler extends ProxyHandler {
    ...
    public static final String CLIENT_INFO = "X-Presto-Client-Info";

    public static final String PREPARED_STATEMENT = "X-Presto-Prepared-Statement";

    public static final String STATEMENT = "X-Presto-Statement";
    public static final long COUNT_RULE = 100;
    ...
    public QueryIdCachingProxyHandler(
            ...
            TapdbCountRuleAggregateManager countRuleAggregateManager,
            ClusterAnalyseRoutingManager clusterAnalyseRoutingManager,
            TapdbQueryFailureInfoManager failureInfoManager,
        ...
        this.countRuleAggregateManager = countRuleAggregateManager;
        this.clusterAnalyseRoutingManager = clusterAnalyseRoutingManager;
        this.failureInfoManager = failureInfoManager;
    ...
    public String rewriteTarget(HttpServletRequest request) {
        ...
        if (isPathWhiteListed(request.getRequestURI())) {
            ...
            if (!Strings.isNullOrEmpty(queryId)) {
                ...
                // 通过查询该项目 event_analyse_routing 路由表，然后路由到对应的项目集群地址进行查询
                String appId = getAppIdByClientInfo(request);
                String clusterRoutingGroup = clusterAnalyseRoutingManager.findClusterRouting(appId);
                if (null != clusterRoutingGroup) {
                    ((MultiReadHttpServletRequest) request).addHeader(ALTERNATE_ROUTING_GROUP_HEADER, clusterRoutingGroup);
                }
                log.info("add cluster routing group request X-Presto-Routing-Group header : [{}]", clusterRoutingGroup);
    ...
    protected void postConnectionHook(
        ...
        try {
            ...
            String appId = getAppIdByClientInfo(request);
            String requestSql = CharStreams.toString(request.getReader());

            String queryId = null;
            if (buffer.length > 0) {
                String output = new String(buffer);
                JSONObject jsonObject = JSONObject.parseObject(output);
                queryId = jsonObject.containsKey("id") ? jsonObject.getString("id") : null;
            }

            // 放行 Select X 的查询和不含有 ClientInfo 的信息的请求查询
            if (isExistClientInfo(request) && !requestSql.equals(VALIDATION_QUERY)) {
                String clusterRoutingGroup = request.getHeader(ALTERNATE_ROUTING_GROUP_HEADER);
                Map<String, ClusterStats> clusterStatsInfo = ActiveClusterMonitor.getClusterStatsInfo();

                if (clusterStatsInfo.containsKey(clusterRoutingGroup)) {
                    ClusterStats clusterStats = clusterStatsInfo.get(clusterRoutingGroup);

                    if (clusterStats.getQueuedQueryCount() >= 30) {
                        // TODO 策略1：按照查询频率进行拦截， 前提是集群负载较高时，再进行进行下一步的查询频率规则校验
                        long queryCountSum = countRuleAggregateManager.getQueryCountSumByAppId(appId);
                        if (queryCountSum > COUNT_RULE) {
                            log.info("Illegal query request intercepted, the request info appId->[{}], queryId->[{}], querySql->[{}]", appId, queryId, requestSql);
                            response.sendError(HttpServletResponse.SC_FORBIDDEN , "Maybe a dangerous request, Please send the request again later.");
                            return;
                        }
            ...
            if (requestPath.startsWith(V1_STATEMENT_PATH)
                ...
                // filter validation query, and not save query history
                if (!requestSql.equals(VALIDATION_QUERY)) {
    ...
    private QueryHistoryManager.QueryDetail getQueryDetailsFromRequest(HttpServletRequest request)
        ...
        // 请求中包含 appId 才会返回结果
        String appId = getAppIdByClientInfo(request);
        queryDetail.setAppId(appId);
        return queryDetail;
    }


    /**
     * 通过 request 请求，获取 clientInfo 中的 appId 信息
     *
     * @param request
     * @return
     */
    public static String getAppIdByClientInfo(HttpServletRequest request) {
        String clientInfo = request.getHeader(CLIENT_INFO);
        String appId = null;

        JSONObject clientInfoObject = JSONObject.parseObject(clientInfo);
        if (null != clientInfoObject && clientInfoObject.containsKey("appId")) {
            appId = (String) clientInfoObject.get("appId");
        }
    ...
    public static boolean isExistClientInfo(HttpServletRequest request) {
        String clientInfo = request.getHeader(CLIENT_INFO);
        return null != clientInfo;
    }
